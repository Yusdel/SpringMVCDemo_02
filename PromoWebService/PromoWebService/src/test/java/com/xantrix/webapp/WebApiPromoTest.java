...
@ContextConfiguration(classes = Application.class) // because we start our application to tested
@SpringBootTest 
@FixMethodOrder(MethodSorters.NAME_ASCENDING) // to specify the run test ordering (ASCENDING = alphabetical ordering)
public class WebApiPromoTest 
{
	private MockMvc mockMvc; // mock element
	
	@Autowired
	private WebApplicationContext wac; // to start the simulation of the application
	
	@Autowired
	private PromoRepository promoRepository;
	
	/*
	 * @Before = because this is not a test but a configuration method for testing.
	 * ( Before starting the test, perform this method )
	 */
	@Before
	public void setup()
	{
		// create moke to run the application in test mode.
		// represent the Context of the application
	...
	public void C_testlistPromoById() throws Exception
	{
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/promo/id/" + promo.getIdPromo())
				...
				.andExpect(jsonPath("$.dettPromo[0].riga").value("2"))
				.andExpect(jsonPath("$.dettPromo[0].codart").exists())
				.andExpect(jsonPath("$.dettPromo[0].codart").value("000020030"))
